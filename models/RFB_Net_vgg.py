...
class RFBNet(nn.Module):
    ...
    def forward(self, x, feed_conv_4_3):
        ...
        conf_source = self.classifier_source(embedding_source_output) * self.scale_source
        conf_source = conf_source.view(conf.shape[0], -1)


        if self.phase == "test":
            output = (
                loc.view(loc.size(0), -1, 4),                   # loc preds
                self.softmax(conf_source.view(-1, self.num_classes)),  # conf preds
                self.softmax_binary(bin_conf.view(-1, 2)),      # bin_conf preds
            )
        else:
            output = (
                loc.view(loc.size(0), -1, 4),
                conf_source.view(conf_source.size(0), -1, self.num_classes),
