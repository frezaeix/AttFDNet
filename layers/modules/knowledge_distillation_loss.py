...
class KD_loss(nn.Module):
    ...
    def __init__(self, num_classes, num_new_classes, overlap_thresh,prior_for_matching,bkg_label,neg_mining,neg_pos,neg_overlap,encode_target):
    ...
    '''
    def forward(self, source_out, target1_out, priors, targets, pos, neg_binary, neg_multi):
        ...
        #origin_conf_data_target = conf_data_target
        conf_data_target = conf_data_target[:,:,:self.num_classes+self.num_new_classes]
        conf_data = conf_data[:,:,:self.num_classes]

        '''

        conf_data_score = F.softmax(conf_data, dim=-1)
        object_score = 1 - conf_data_score[:,:,0]
        _, pos_index = object_score.topk(256, dim=1)
        pos = pos > 2
        for index in range(pos.shape[0]):
            pos[index, pos_index] = 1
        '''
        ...
        P_logit = batch_conf.view(num,-1,self.num_classes)
        ...
        batch_conf_target = conf_data_target.view(-1, self.num_classes + self.num_new_classes)
        ...
        '''
        P_logit_target = batch_conf_target.view(num,-1,self.num_classes + self.num_new_classes)
        ...
        P_logit_multi_target = P_logit_target[(pos_idx+neg_idx).gt(0)].view(-1, self.num_classes + self.num_new_classes)
        ...
        P_logit_multi_target = P_logit_multi_target[:,:self.num_classes]
